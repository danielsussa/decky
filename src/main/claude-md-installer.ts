...
- **Descobrir o que o usuário tem aberto**: as tabs abertas no decky são contexto vivo que o Claude não enxerga automaticamente. Chame \`list_cards\` (e, se fizer sentido, \`Read\` no path do focused) **proativamente** quando: (1) a pergunta tem pouco contexto e parece esperar contexto prévio — ex "vamos voltar pra esse assunto?", "e aí?", "continua", "o que falta?" sem antecedente nesta conversa; (2) a pergunta referencia algo via demonstrativo ambíguo — "esse / essa / aquele / isso aí / o doc / aquela lista / o card" sem você ter id na mão; (3) o pedido é editar/atualizar algo que claramente não é desta conversa. Retorno: id, type, title, path (quando aplicável), focused, pinned. Card focado é o referente mais provável; pra cards markdown o \`path\` retornado já é editável com \`Edit\`. **Não** chame se a pergunta é autocontida (instrução clara, contexto suficiente nesta conversa, trabalho técnico em arquivos do código).
