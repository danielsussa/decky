...
export function registerCardMirrorHandlers(_getWindow: () => BrowserWindow | null): void {
  // Renderer pushes its current per-session snapshot (debounced on its side).
  ipcMain.on('cards:state-sync', (_e, payload: { sessions: Record<string, MirrorSession> }) => {
  ...
  // Renderer acks a preview broadcast: tells main which cardId+path the inline source
  // landed on so the HTTP response can echo it back to the MCP caller.
  ipcMain.on(
    'preview:resolved',
    (_e, payload: { reqId?: string; cardId?: string; path?: string; title?: string }) => {
