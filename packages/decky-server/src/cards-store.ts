...
export async function writeCard(
  ...
  const path = cardFilePath(workspace, cardId, ext)
...
// List every .md/.html page under the workspace's cards dir (recursive). Returns id (slash-
// segmented), abs path, derived title (first heading or first non-empty line), and mtime.
export async function listCards(workspace: string): Promise<WorkspaceCardEntry[]> {
...
export async function deleteCard(workspace: string, cardId: string): Promise<boolean> {
