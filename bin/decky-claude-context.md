...
### Driving a web page — use decky's OWN browser, not handoff

The decky panel can hold a **web card** (a real browser view inside the shell), and the MCP gives you native tools to drive it: **`browser_navigate`** (open a URL/domain/search in the panel — navigates the focused web card or opens a new one), **`browser_snapshot`** (list visible interactive elements, each with a stable `ref`), **`browser_click`** / **`browser_type`** (act on a `ref` from the latest snapshot), **`browser_read`** (URL + title + visible text), **`browser_eval`** (diagnostic JS in the page).

- **This is decky's browser — prefer it for anything happening in the panel.** When the user points at a page open in the shell ("control the browser open here", "continue from this page"), that IS a web card → drive it with `browser_*`. Do **NOT** reach for `handoff`/`me` to open a separate Chromium for it.
- **Logins persist.** Web cards share a persistent session (`persist:deckweb`), so a site the user already logged into (Meta, etc.) stays logged in across web cards — you continue from where they are, no fresh login.
- **Loop**: `browser_navigate` → `browser_snapshot` (fresh refs) → `browser_click`/`browser_type` → snapshot again. Refs go stale after navigation, so re-snapshot.
- **When `handoff`/`me` is still right**: only for browser work that genuinely isn't in the decky panel — the `me` daemon's own Live View flows, or a site that must run in handoff's separate Chromium profile. The global handoff guidance does **not** override this: inside a decky session, in-panel browsing goes through `browser_*` first.
